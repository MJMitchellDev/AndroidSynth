package com.mjmitchelldev.androidsynth
...
class NativeWavetableSynthesizer : WavetableSynthesizer, DefaultLifecycleObserver {
    ...
    private external fun setFilterCutoffFrequency(synthesizerHandle: Long, frequencyInHz: Float)
    ...
    override suspend fun setFilterCutoffFrequency(frequencyInHz: Float) = withContext(Dispatchers.Default) {
        synchronized(synthesizerMutex) {
            createNativeHandleIfNotExists()
            setFilterCutoffFrequency(synthesizerHandle, frequencyInHz)
        }
    }
